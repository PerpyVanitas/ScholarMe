...
describe("SignUp Page UI", () => {
  ...
  it("should prevent double-click signup by disabling the button", async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Degree Program/i), { target: { value: "BS CS" } });
    fireEvent.change(screen.getByLabelText(/Year Level/i), { target: { value: "1" } });
  ...
  it("should show appropriate error toast when login/signup fails", async () => {
    // @ts-ignore
    vi.spyOn(authActions, "signUp").mockResolvedValue({ error: "Invalid email" });
