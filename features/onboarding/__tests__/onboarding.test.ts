...
describe("Onboarding & Profile Completion (Phase 3)", () => {
  it("P3-2: Flags missing required profile fields (degree_program, year_level, membership_number)", () => {
    const validateProfile = (data: any, isTutor: boolean) => {
      ...
      if (isTutor && !data.membership_number?.trim()) {
    ...
    const learnerErrors = validateProfile({ degree_program: "", year_level: null }, false);
    ...
    const tutorErrors = validateProfile({ degree_program: "BS CS", year_level: 3, membership_number: "" }, true);
    expect(tutorErrors.membership_number).toBeDefined();

    // Valid Tutor
    const validTutor = validateProfile({ degree_program: "BS CS", year_level: 3, membership_number: "2024-001" }, true);
