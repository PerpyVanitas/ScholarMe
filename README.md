...
├── mobile/                 # React Native / Expo codebase (cross-platform prototyping)
...
3. **Database**:
   Run the scripts in `supabase/migrations/` sequentially in your Supabase SQL Editor to define the schema and tables.
...
3. **QR Verification**: Every user has a unique **Digital ID Card** tied to their Student ID number. Admins use the **Identity Scanner** (Android) to verify status, XP, active sessions, and Honor Society designations instantly.
