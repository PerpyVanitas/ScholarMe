...
- **Emergency Out-of-Pocket Reimbursement Claim Dialog (`features/finance/components/emergency-reimbursement-dialog.tsx`)**: Created claim submission modal for urgent expenses personally advanced when organizational funds could not be released in time.
- **Lost Receipt Explanation & Affidavit Submission Dialog (`features/finance/components/lost-receipt-dialog.tsx`)**: Built affidavit intake modal for missing receipts with sworn explanation, alternative digital payment proof, and audit warning notice.
- **Compliance Flag Appeal Submission Modal (`features/finance/components/appeal-submission-modal.tsx`)**: Created 3-day written appeal submission portal for officers issued Orange or Red compliance flags.
- **Financial Summary Report Exporter (`features/finance/components/financial-report-exporter.tsx`)**: Built client-side CSV exporter and print formatter for semester financial statements and SCARDS.
- **Financial Notification Dispatcher Utility (`lib/utils/finance-notifications.ts`)**: Built notification helper and unit test suite (`finance-notifications.test.ts`) for flag issuance, late liquidations, and petty cash replenishment alerts.
- **RBAC Strict Role Gate Compliance Audit (`docs/rbac.md`)**: Conducted system-wide RBAC compliance audit. Refactored direct `role === "tutor"` equality checks in `app/dashboard/resources/page.tsx`, `app/dashboard/sessions/page.tsx`, and `app/dashboard/network/users/[id]/page.tsx` to use `hasAnyRole(role, TUTOR_ROLES)` so executive officers and committee heads properly retain tutor management privileges as mandated by RBAC Rule 1.
- **Honor Society Finance Policy & Audit System Integration (`supabase/migrations/20260803000000_finance_policy_system.sql`)**: Implemented database migration for compliance flags, whistleblower intake, dual-verified revenue collections, investigation case management, supplemental budget requests, and ₱300 petty cash limits.
- **Supplemental Budget Request API (`app/api/v1/finance/supplemental/route.ts`)**: Built endpoint and dialog component for requesting additional budget with automated variance % calculation (>10% warning trigger) and Vitest suite (`route.test.ts`).
- **Compliance Flagging API & Register (`app/api/v1/finance/flags/route.ts`)**: Created Auditor endpoint and dashboard register component for issuing Yellow, Orange, and Red compliance flags with Vitest suite (`route.test.ts`).
- **Confidential Whistleblower Portal (`app/api/v1/finance/whistleblower/route.ts`)**: Added anonymous whistleblower intake endpoint and dashboard tab with submitter ID masking and Vitest suite (`route.test.ts`).
- **Dual-Officer Revenue Collections Ledger (`app/api/v1/finance/collections/route.ts`)**: Built income collection logging route requiring 2-officer verification and deposit tracking with Vitest suite (`route.test.ts`).
- **Committee on Investigation Case Manager (`app/api/v1/finance/investigations/route.ts`)**: Created investigation case logging endpoint and dashboard tab for referral evidence evaluation and Vitest suite (`route.test.ts`).
- **Conflict of Interest Approver Declaration Modal (`features/finance/components/coi-declaration-modal.tsx`)**: Created mandatory COI modal forcing approvers to declare "No Conflict" or "Conflict Exists" with alternate escalation.
- **Transaction Lifecycle Progress Tracker (`features/finance/components/workflow-progress-tracker.tsx`)**: Created 10-stage horizontal step visualizer tracking financial transactions from submission to archiving.
- **Support Ticket Escalation API Endpoint (`app/api/v1/admin/support/[id]/escalate/route.ts`)**: Created 1-click support ticket escalation route handler to route complex issues directly to Super Admin queues with unit test suite (`route.test.ts`).
- **Account Inactivity Pruning Helper (`lib/utils/account-pruning.ts`)**: Added stale account identification utility for accounts inactive > 180 days with unit test suite (`account-pruning.test.ts`).
- **Anki (.apkg) & CSV Flashcard Exporter (`lib/utils/flashcard-export.ts`)**: Built export formatter utility converting flashcards to Anki CSV formats with unit test suite (`flashcard-export.test.ts`).
- **Secretariat Meeting Minutes Markdown Formatter (`lib/utils/minutes-formatter.ts`)**: Created meeting minutes formatter generating structured markdown/HTML with unit test suite (`minutes-formatter.test.ts`).
...
- **PLC Live Desk Check-In Route (`components/plc-live-desk-widget.tsx`)**:  
